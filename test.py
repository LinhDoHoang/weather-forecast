...
def preprocess_from_scratch(
    ...
    encoders = {}
    for cat_col in ["province", "wind_d", "region"]:
    ...
    if "province" in df.columns:
        df["province_encoded"] = df["province"]
    ...
    feature_cols = [c for c in train_df.columns if c not in non_feature_cols and c != target_col]
